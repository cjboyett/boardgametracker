...
public class PlayerDataAdapter extends BaseAdapter {
	...
	public PlayerDataAdapter(final Activity activity, Collection<GamePlayerData> gamePlayerData, boolean countWins) {
		this(activity,
			 gamePlayerData,
			 countWins,
			 (Preferences.generatePalette(activity) ? Preferences.getGeneratedBackgroundColor(activity) : Preferences.getBackgroundColor(
					 activity)),
			 (Preferences.generatePalette(activity) ? Preferences.getGeneratedForegroundColor(activity) : Preferences.getForegroundColor(
					 activity)));
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (countWins) {
			if (gamePlayerData.isWin()) {
				...
				ViewUtilities.tintImageView((AppCompatImageView) view.findViewById(R.id.imageview_win_icon),
											foregroundColor);
			} else
