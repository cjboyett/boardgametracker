...
public class BoardGame extends Game {
	...
	public static BoardGame createGame(BoardGameXmlParser.Item item) {
		...
		for (String[] extra : item.links) {
			if (extra[0].equals("boardgamecategory"))
				game.addCategory(new BoardGameCategory(extra[2], Integer.parseInt(extra[1])));
			else if (extra[0].equals("boardgamefamily"))
				game.addFamily(new BoardGameFamily(extra[2], Integer.parseInt(extra[1])));
			else if (extra[0].equals("boardgamemechanic"))
				game.addMechanic(new BoardGameMechanic(extra[2], Integer.parseInt(extra[1])));
			else if (extra[0].equals("boardgamepublisher"))
				game.addPublisher(new BoardGamePublisher(extra[2], Integer.parseInt(extra[1])));
			else if (extra[0].equals("boardgameexpansion"))
				game.addExpansion(new BoardGame(extra[2], null, Integer.parseInt(extra[1])));
		}
