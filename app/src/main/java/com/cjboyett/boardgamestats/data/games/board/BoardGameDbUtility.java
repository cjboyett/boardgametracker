...
public class BoardGameDbUtility
{
	public static void addBoardGame(GamesDbHelper dbHelper, String gameName, String description, int yearPublished,
	                                int bggid, String thumbnailUrl, String expansions, String categories, String families,
	...
	{
		String expansions = "";
		for (BoardGame expansion : game.getExpansions()) expansions += expansion.getName() + "|" + expansion.getBggId() + "|";
		if (expansions.endsWith("|")) expansions = expansions.substring(0,expansions.length()-1);
		addBoardGame(dbHelper, game.getName(), game.getDescription(), game.getYearPublished(),
				game.getBggId(), game.getThumbnailUrl(), expansions,
				game.getCategories().toString(), game.getFamilies().toString(), game.getMechanics().toString(), "boardgame");
	}
	...
	{
		String expansions = "";
		for (BoardGame expansion : boardGame.getExpansions()) expansions += expansion.getName() + "|" + expansion.getBggId() + "|";
		if (expansions.endsWith("|")) expansions = expansions.substring(0,expansions.length()-1);
		return updateBoardGame(dbHelper, oldGameName, boardGame.getName(), boardGame.getDescription(), boardGame.getYearPublished(),
				boardGame.getBggId(), boardGame.getThumbnailUrl(), expansions,
				boardGame.getCategories().toString(), boardGame.getFamilies().toString(), boardGame.getMechanics().toString(), "boardgame");
	}

	public static boolean updateBoardGame(GamesDbHelper dbHelper, String oldGameName, String gameName, String description, int yearPublished,
	                                   int bggid, String thumbnailUrl, String expansions, String categories, String families,
	                                   String mechanics, String gameType)
	{
		ContentValues values = new ContentValues();
	...
	{
		addGamePlay(dbHelper, boardGamePlayData.getGame().getName(), boardGamePlayData.getScore(), boardGamePlayData.isWin() ? 'y' : 'n',
					boardGamePlayData.getTimePlayed(), boardGamePlayData.getDate().rawDate(), boardGamePlayData.getNotes(),
					boardGamePlayData.getLocation(), boardGamePlayData.isCountForStats(), boardGamePlayData.getImages(),
				    boardGamePlayData.getOtherPlayers().values(), boardGamePlayData.getBggPlayId());
	}
	...
	public static void updateGamePlay(GamesDbHelper dbHelper, long gamePlayId, BoardGamePlayData boardGamePlayData)
	{
		updateGamePlay(dbHelper, gamePlayId, boardGamePlayData.getGame().getName(), boardGamePlayData.getScore(),
				boardGamePlayData.isWin() ? 'y' : 'n', boardGamePlayData.getTimePlayed(), boardGamePlayData.getDate().rawDate(),
				boardGamePlayData.getNotes(), boardGamePlayData.getLocation(), boardGamePlayData.isCountForStats(),
				boardGamePlayData.getImages(), boardGamePlayData.getOtherPlayers().values());
	}
	...
	public static BoardGamePlayData getGamePlay(GamesDbHelper dbHelper, long gamePlayId)
	{
		...
		{
			GamePlayerData gamePlayerData = new GamePlayerData(playersCursor.getString(0), playersCursor.getDouble(1), playersCursor.getString(2).equals("y"));
			...
			while (playCursor.moveToNext())
			{
				...
				while (playersCursor.moveToNext())
				{
					GamePlayerData gamePlayerData =
							new GamePlayerData(playersCursor.getString(0), playersCursor.getDouble(1), playersCursor.getString(2).equals("y"));
					boardGamePlayData.addOtherPlayer(gamePlayerData.getPlayerName(), gamePlayerData);
				}
				playersCursor.close();
	...
	public static List<HotnessXmlParser.Item> getHotnessItems(GamesDbHelper dbHelper)
	{
		...
		{
			items.add(new HotnessXmlParser.Item(hotnessCursor.getString(0), hotnessCursor.getInt(1), hotnessCursor.getInt(2), hotnessCursor.getString(3)));
		}
		hotnessCursor.close();
