...
public class PlayersDbUtility {
	...
	public static void addPlayerData(GamesDbHelper dbHelper, PlayerData playerData) {
		...
		try {
			dbHelper.getWritableDatabase()
					.insertWithOnConflict(PlayerEntry.TABLE_NAME, null, values, SQLiteDatabase.CONFLICT_REPLACE);
		} catch (Exception e) {
			dbHelper.getWritableDatabase()
					.update(PlayerEntry.TABLE_NAME,
							values,
							PlayerEntry.NAME + " = ?",
							new String[]{playerData.getName()});
		}
	}
	...
	public static void setMostPlayedGameByTimesWith(GamesDbHelper dbHelper, String player,
													String mostPlayedGameByTimes) {
		ContentValues values = new ContentValues();
	...
	public static void generateNewPlayer(GamesDbHelper dbHelper, String player) {
		List<GamePlayData> gamePlayDataList =
				GameStatsDbUtility.getGamePlaysWithPlayer(dbHelper, player, true, true, true);
		...
		String mostPlayedGameByTimes = "", mostPlayedGameByTime = "", mostWonGame = "", mostLostGame = "",
				mostTiedGame = "";
		if (!mostPlayedGamesByTimes.isEmpty())
			mostPlayedGameByTimes = mostPlayedGamesByTimes.get(r.nextInt(mostPlayedGamesByTimes.size()));
		if (!mostPlayedGamesByTime.isEmpty())
			mostPlayedGameByTime = mostPlayedGamesByTime.get(r.nextInt(mostPlayedGamesByTime.size()));
	...
	public static String generateBlurb(GamesDbHelper dbHelper, String player) {
		...
		switch (selection) {
			case 0:
				...
				blurb = "<b>Times played with " + player + ":</b><br/>" + timesPlayed + " play" +
						(timesPlayed != 1 ? "s" : "");
				break;
			case 1:
				blurb = "<b>Time played with " + player + ":</b><br/>" +
						StringUtilities.convertMinutes(timePlayedWith(dbHelper, player));
				break;
			case 2:
				blurb = "<b>Favorite game (by count) with " + player + ":</b><br/>" +
						mostPlayedGameByTimesWith(dbHelper, player);
				break;
			case 3:
				blurb = "<b>Favorite game (by time) with " + player + ":</b><br/>" +
						mostPlayedGameByTimeWith(dbHelper, player);
				break;
			case 4:
			...
			case 5:
				...
				break;
			case 6:
				blurb = "<b>Win percentage against " + player + ":</b><br/>" +
						NumberFormat.getPercentInstance().format(winPercentageWith(dbHelper, player) / 100);
				break;
			case 7:
				blurb = "<b>Lose percentage against " + player + ":</b><br/>" +
						NumberFormat.getPercentInstance().format(losePercentageWith(dbHelper, player) / 100);
				break;
		}
		return blurb;
	}
