...
public class PlaysXmlParser
{
	...
	private GamePlayData readItem(XmlPullParser parser)
	{
		...
		{
			case BOARD:
				gamePlayData = new BoardGamePlayData((BoardGame)game, score, win, timePlayed, new Date(date), notes, -1l);
				break;
			case RPG:
				gamePlayData = new RPGPlayData((RolePlayingGame)game, timePlayed, new Date(date), notes, -1l);
				break;
			case VIDEO:
				gamePlayData = new VideoGamePlayData((VideoGame)game, score, win, timePlayed, new Date(date), notes, -1l);
				break;
		}
		if (gamePlayData != null)
		{
			...
			for (GamePlayerData player : players)
			{
				...
				if (gameType == Game.GameType.BOARD)
					if (player.getPlayerName().equals("master_user")) ((BoardGamePlayData)gamePlayData).setWin(player.isWin());
				if (gameType == Game.GameType.VIDEO)
					if (player.getPlayerName().equals("master_user")) ((VideoGamePlayData)gamePlayData).setWin(player.isWin());
			}
		}
	...
	private Game readGame(XmlPullParser parser)
	{
		...
		try
		{
			...
			if (parser.getAttributeValue(null, "objectid") != null && !parser.getAttributeValue(null, "objectid").equals(""))
	...
	private GamePlayerData readPlayer(XmlPullParser parser)
	{
		...
		try
		{
			...
			if (parser.getAttributeValue(null, "username").equals(bggUserName) ||
					parser.getAttributeValue(null, "name").equals(userName)) gamePlayerData.setPlayerName("master_user");
