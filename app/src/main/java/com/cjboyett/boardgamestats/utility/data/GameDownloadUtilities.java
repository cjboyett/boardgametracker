...
public class GameDownloadUtilities
{
						...
						try
						{
								...
								if (!BoardGameDbUtility.gamePlayExists(dbHelper, gamePlayData.getBggPlayId()))
								{
									if (!boardGameIds.contains((long) gamePlayData.getGame().getBggId()))
									{
										gamePlayData.setGame(new DownloadXmlGameTask(gameType)
												.execute("https://www.boardgamegeek.com/xmlapi2/thing?id=" + gamePlayData.getGame().getBggId())
												.get());
										GameDownloadUtilities.downloadThumbnail(gamePlayData.getGame().getThumbnailUrl(), context);
							...
							} else if (gameType == Game.GameType.RPG)
							{
								if (!RPGDbUtility.gamePlayExists(dbHelper, gamePlayData.getBggPlayId()))
								{
									...
									{
										gamePlayData.setGame(new DownloadXmlGameTask(gameType)
												.execute("https://www.boardgamegeek.com/xmlapi2/family?id=" + gamePlayData.getGame().getBggId())
												.get());
										GameDownloadUtilities.downloadThumbnail(gamePlayData.getGame().getThumbnailUrl(), context);
	...
	private static List<GamePlayData> loadGamePlayDataXmlFromNetwork(Context context, String urlString, Game.GameType gameType)
	{
		InputStream in = null;
