package com.cjboyett.boardgamestats.conductor.addgame;
...
public class AddGameController extends BaseController implements AddGameView {
	...
	public AddGameController(boolean sync, String oldGameName, String gameType) {
		this.sync = sync;
		this.oldGameName = oldGameName;
		if (StringUtilities.isBoardGame(gameType)) {
			this.gameType = Game.GameType.BOARD;
		} else if (StringUtilities.isRPG(gameType)) {
			this.gameType = Game.GameType.RPG;
		} else if (StringUtilities.isVideoGame(gameType)) {
			this.gameType = Game.GameType.VIDEO;
		}
	}

	public AddGameController(String gameName, String gameType) {
	...
	protected View onCreateView(@NonNull LayoutInflater inflater, @NonNull ViewGroup container) {
		view = inflater.inflate(R.layout.content_add_game, container, false);
//		ButterKnife.bind(this, view);
		presenter = new AddGamePresenter();
		return view;
	}
	...
	protected void onAttach(@NonNull View view) {
		...
		if (!TextUtils.isEmpty(gameName)) {
			gameSearchView.setQuery(gameName, true);
		}
	}
	...
	private void setRPGListAdapter(Map<Integer, String> rpgs) {
		...
		listView.setAdapter(new CustomArrayAdapter(activity, android.R.layout.simple_list_item_1, games));
	...
	private void setBoardGameListAdapter(List<BoardGameXmlParser.Item> items) {
		...
		if (boardGamesList.isEmpty()) {
			...
			listView.setAdapter(new CustomArrayAdapter(activity, android.R.layout.simple_list_item_1, makeList()));
			listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					new DownloadXmlGameTask().execute(
	...
	private void showAddManualDialog() {
		...
		gameTypeSpinner.setAdapter(new CustomArrayAdapter(activity,
...
//		if (getIntent().hasExtra("SEARCH")) gameSearchView.setQuery(getIntent().getStringExtra("SEARCH"), true);
		...
		ArrayAdapter<String> adapter =
				new CustomArrayAdapter(activity, android.R.layout.simple_list_item_1, makeList());
